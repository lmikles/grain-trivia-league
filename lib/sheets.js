 ...
 * @param {string}   sheetName  Tab name only (e.g. 'QuestionLog')
 * @param {Array[][]} rows       2-D array of rows to append
 */
...
/**
 * Create a new sheet tab. Silently ignores the error if the tab already exists.
...
async function createSheetTab(title) {
  ...
  try {
    ...
    // "already exists" error is expected after first run — ignore it
    if (!err.message?.includes('already exists')) throw err;
  }
}

module.exports = { readRange, appendRow, appendRows, updateRange, clearRange, createSheetTab };
